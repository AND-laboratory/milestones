...
# Calculate total counts, percentages, mean age, and standard deviation of age for each profile
profile_summary <- GZMCore_Weighted %>%
  group_by(C) %>%
  summarise(
    TotalCount = n(),
    Percentage = (TotalCount / nrow(GZMCore_Weighted)) * 100,
    MeanAge = mean(AgeCurrent, na.rm = TRUE),
    SDAge = sd(AgeCurrent, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  arrange(C)
...
all_plot_data <- all_plot_data %>%
  mutate(
    Profile = factor(Profile, levels = c("Profile 1", "Profile 3", "Profile 2", "Profile 4"))
  )
...
p <- ggplot(all_plot_data, aes(x = Milestone, y = MeanAge, color = Profile, shape = Profile, group = Profile)) +
  ...
  scale_color_brewer(
    palette = "Set1", 
    labels = c(
      bquote("Profile 1 (EA; " * italic(n) * "=100)"), 
      bquote("Profile 3 (MA; " * italic(n) * "=241)"), 
      bquote("Profile 2 (LA; " * italic(n) * "=110)"), 
      bquote("Profile 4 (AH; " * italic(n) * "=39)")
    )
  ) +
  guides(
...
## Non Achievement Plots

```{r non achievement plots}
# Dividing the MeanWeightedProb column by 100 in each data frame
plot_data$MeanWeightedProb <- plot_data$MeanWeightedProb / 100
plot_data2$MeanWeightedProb <- plot_data2$MeanWeightedProb / 100
plot_data3$MeanWeightedProb <- plot_data3$MeanWeightedProb / 100
plot_data4$MeanWeightedProb <- plot_data4$MeanWeightedProb / 100

# Function to create plot
create_plot <- function(data, profile_name) {
  ggplot(data, aes(x = Milestone, y = MeanWeightedProb, fill = MeanWeightedProb)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.5), width = 0.6) +
    geom_hline(yintercept = 0.50, linetype = "dotted", color = "red") +
    scale_fill_gradient(name = "Non-Achievement", low = "lightgray", high = "black", limits = c(0, 1)) +
    scale_y_continuous(labels = scales::percent_format(), limits = c(0, 1)) +
    labs(title = paste("Non-Achievement Proportion -", profile_name)) +
    theme_minimal() +
    theme(
...
# Mapping from profile number to label
profile_labels <- c("Profile 1 (EA)", "Profile 2 (LA)", "Profile 3 (MA)", "Profile 4 (AH)")

# Prepare and store all plots in a list with descriptive titles
plot_list <- lapply(1:4, function(i) {
  profile_data <- get(paste0("plot_data", ifelse(i == 1, "", i)))
  profile_name <- profile_labels[i]
  create_plot(profile_data, profile_name)
})

# Combine all plots using patchwork's operators into a 2x2 grid
combined_plot <- (plot_list[[1]] | plot_list[[2]]) /
  (plot_list[[3]] | plot_list[[4]]) +
  plot_layout(guides = 'collect')  # Collect all guides into a single global legend

# Add global X and Y labels
combined_plot <- combined_plot + 
  labs(x = "Milestone", y = "Proportion of Non-Achievement (%)") +
...
# Save the plot as a PDF
ggsave("non-achievement_plot.pdf", plot = combined_plot, width = 15, height = 10)
```
